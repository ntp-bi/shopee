		...
		try {
			...
			if (un == null && pass == null && name == null) {
				...
				if (n) {
					...
					RequestDispatcher rd = request.getRequestDispatcher("AdminRegister.jsp?war=Đăng ký thành công!");
					rd.forward(request, response);

				} else {
					RequestDispatcher rd = request.getRequestDispatcher(
							"AdminRegister.jsp?war=Tài khoản đã tồn tại hoặc chưa nhập đầy đủ thông tin!");
					rd.forward(request, response);
				}
			}
		} catch (Exception e) {
